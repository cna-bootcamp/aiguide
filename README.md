...
- '지침'에 목표, 팀 행동원칙, 팀원, 대화 가이드, 최적안 도출 가이드 등 AI와의 협업에 필요한 지침 설정   
  ...
  '[작업지침]'의 {프로젝트}는 사전작업에서 작성한 이름 지정    
  ...
  [작업지침]
  - 작업결과는 {사용자홈}/home/workspace/{프로젝트}에 생성  
  ```
...
```
[요청사항]
- 가장 큰 현상문제 3개를 도출하고 5WHY기법으로 근본문제를 도출해 주세요.
- {현상문제|WHY1|WHY2|WHY3|WHY4|WHY5|근본원인}형식으로 작성해 주세요.   
- 근본원인을 너무 기술적 솔루션으로 생각하지 말고 사용자의 심리적 요인에서 찾음. 
[참고자료]
- 시장조사
- 관찰결과
- 체험결과
...
### 문제가설 검증 
- 'sample_문제검증인터뷰 결과.docx'를 파일에 업로드   
- 프롬프트 예시  
```
[요청사항]
...
### 문제검증인터뷰 결과 취합  
- 'sample_문제검증인터뷰 결과취합.xlsx'를 pdf로 변환하여 파일에 업로드  
- 위 단계 수행결과 결과 '문제검증인터뷰 결과.docx'를 파일에 업로드 
- 프롬프트 예시  
```
[요청사항]
...
- 마크다운 표
```  
- '문제검증인터뷰 결과취합'이라는 제목으로 파일에 업로드 합니다.  
...
- 수렴 프롬프트 예시
  ```
  [요청사항]
  - {최적안 가이드}에 따라 유사하거나 동일한 것은 합쳐서 정리
  - 첫줄: 아이디어 구분: 아이디어 제목, 둘째줄: 아이디어 설명 
  - 아이디어 설명은 최대한 자세히 표현
  - 각 아이디어를 복사하기 쉽게 코드블록으로 분리하여 작성
  [결과형식]
  코드블록
  ``` 
...
- 선정된 아이디어를 '핵심솔루션'이라는 제목으로 파일에 업로드 
  ```
  [요청사항]
  - 아래 핵심솔루션을 표 형식으로 정리
    - 통합 구독 관리 인프라
