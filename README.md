...
## 🚀 Claude Skills 자동화 (신규)

**Claude Code**를 사용하면 MVP 주제만 제시하면 자동으로 전체 기획 프로세스가 실행됩니다!

### 빠른 시작

```
"음식 배달 서비스 MVP를 처음부터 끝까지 기획해줘"
```

Claude가 자동으로:
- ✅ 13개 기획 단계를 순차적으로 실행
- ✅ MVP 정의 → 고객분석 → 시장조사 → ... → 프로토타입까지
- ✅ 모든 결과를 `outputs/` 디렉토리에 저장

### 상세 가이드

- 📖 **[Claude Skills 완전 가이드](README-SKILLS.md)** - 전체 사용법 및 모든 기능
- ⚡ **[빠른 시작 가이드](QUICKSTART-SKILLS.md)** - 5분 안에 시작하기

### 기존 수동 프로세스

아래 내용은 기존 수동 프로세스 가이드입니다. Claude Skills를 사용하지 않고 단계별로 직접 진행하고 싶을 때 참고하세요.
...
- 개선/수정 요청  
  PlayWright MCP를 이용하여 브라우저에서 화면을 열어 수정하도록 요청 합니다.   
  예)
  ```
  트리구조탐색화면을 웹브라우저로 열어봐요. 좌측 이미지 컬렉션이 이상하게 나오네요.
  ```
