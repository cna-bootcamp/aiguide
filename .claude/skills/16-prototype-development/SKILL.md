...
Playwright MCP를 이용하여 브라우저에서 화면을 열어 테스트합니다.
...
메시지 크기 초과로 개발이 중단된 경우:
1. 새 대화창을 엽니다
2. 위의 "프롬프팅 구조" 섹션의 프롬프트를 복사합니다
3. 새 대화창에 붙여넣습니다
4. Claude가 기존 개발된 프로토타입을 검토하여 계속 진행합니다

**프롬프트 예시**:
```
design/uiux/prototype 디렉토리에 작성중인 프로토타입을 먼저 파악한 후
이어서 개발해 주세요.

[요청사항]
- 기존에 개발된 화면을 확인하고 이어서 개발
- 샘플 데이터와 스타일의 일관성 유지
- <작성원칙>과 <작성순서>를 준수
```
...
### 1. 웹브라우저에서 확인
- 생성된 프로토타입 파일을 웹브라우저에서 엽니다
- 개발자 도구(F12)를 열어 콘솔 창을 확인합니다
- 에러 메시지나 경고가 있는지 확인합니다

### 2. 에러 수정
**에러가 있는 경우**:
- 콘솔 에러 메시지를 복사합니다
- 에러 메시지와 함께 수정을 요청합니다

**프롬프트 예시**:
```
05-홈.html 파일을 열었더니 콘솔에 다음 에러가 나옵니다. 수정해 주세요.
...
### 3. 개선 요청
**화면이 예상과 다른 경우**:
- Playwright MCP를 이용하여 브라우저에서 화면을 열어 확인합니다
- 구체적으로 어떤 부분을 개선할지 설명합니다
...
### Playwright MCP 사용
프로토타입 테스트를 위해 Playwright MCP를 활용하여:
- 브라우저에서 자동으로 프로토타입 열기
- 화면 스크린샷 캡처
- 인터랙션 테스트 자동화
- 다양한 화면 크기에서 테스트
