...
| **커맨드 [커맨드]** | Story Title/Action | 사용자가 원하는 행동 |
| **이벤트 [이벤트]** | Acceptance Criteria | 행동의 결과로 기대하는 상태 |
| **정책/규칙 [정책/규칙]** | Acceptance Criteria (조건) | 비즈니스 규칙 및 제약사항 |
| **데이터 [데이터]** | Story 상세 / API 명세 | 입출력 데이터 정의 |
...
**변환 공식**:
{사용자 유형}으로서 | 나는, {이벤트로 달성하려는 목적}을 위해 | {커맨드의 행동}을 원한다.

**예시 (01-회원가입.puml 기준)**:
```
`[커맨드] 카카오 로그인 버튼 클릭` -> 판매자/구매자로서 | 나는, 복잡한 입력 없이 빠르게 서비스를 이용하기 위해 | 카카오 계정으로 간편하게 회원가입하고 싶다.   
`[커맨드] 이메일/비밀번호 입력` -> 판매자/구매자로서 | 나는, SNS 계정 없이도 서비스를 이용하기 위해 | 이메일로 회원가입하고 싶다.
```

### 4. 정책/규칙 → Acceptance Criteria 변환
