...
description: 비즈니스 도메인만 입력하면 MVP 정의부터 프로토타입까지 전체 서비스 기획 워크플로우를 자동으로 실행합니다. 16개의 전문화된 스킬을 순차적으로 orchestrate하여 완전한 기획 산출물을 생성합니다.
---
...
이 skill은 6개 단계에서 16개의 전문화된 스킬을 orchestrate합니다:
...
**Step 2.1: 시장 조사**
```
03-market-research skill 호출
입력: MVP 주제 + 고객 분석 (define/MVP정의.md, define/고객분석.md)
출력: 시장 분석, 경쟁 현황, 트렌드
저장: define/시장조사.md
```
...
**Step 2.2: 고객경험 단계 정의**
```
04-customer-journey-stages skill 호출
입력: MVP 주제 + 고객 분석 + 시장 조사
...
**Step 2.3: 고객 경험 조사**
```
05-customer-experience skill 호출
입력: 고객 페르소나 + 고객경험 단계 (define/고객분석.md, define/고객경험단계.md)
...
**Step 2.4: 고객 여정 맵 작성**
```
06-journey-mapping skill 호출
입력: 고객 분석 + 경험 데이터 + 고객경험 단계
...
**Step 2.5: 문제 가설 정의**
```
07-problem-hypothesis skill 호출
입력: 고객 여정 맵 인사이트 (define/유저저니맵.md)
출력: 해결해야 할 핵심 문제 진술
...
**Step 2.6: 방향성 정의**
```
08-direction-setting skill 호출
입력: 문제 가설 + 고객 고통점 (define/문제가설.md, define/고객경험인터뷰결과취합.md)
...
**Step 3.1: 아이디어 발상**
```
09-ideation skill 호출
입력: 문제 가설 (define/문제가설.md, think/킹핀문제.md, think/문제해결방향성.md)
...
**Step 3.2: 솔루션 선정**
```
10-solution-selection skill 호출
입력: 아이디어 발상 결과 (think/솔루션탐색.md, think/솔루션후보.md)
...
```
11-business-modeling skill 호출
입력: 선정된 솔루션 + 고객 분석 (think/핵심솔루션.md, define/고객분석.md, define/문제가설.md)
출력: 완전한 린 캔버스
저장: think/비즈니스모델.md
```
...
**Step 4.2: 발표자료 스크립트**
```
12-presentation skill 호출
입력: 비즈니스 모델 + 모든 이전 산출물
...
**Step 5.1: 이벤트 스토밍**
```
13-event-storming skill 호출
입력: 선정된 솔루션 + 고객 여정 + 방향성 정의 (think/핵심솔루션.md, define/고객분석.md)
...
**Step 5.2: 유저스토리 작성**
```
14-user-stories skill 호출
입력: 이벤트 스토밍 PlantUML 다이어그램 + 고객 여정 (think/핵심솔루션.md, define/고객분석.md, think/es/*.puml)
...
**Step 5.3: UI/UX 설계**
```
15-uiux-design skill 호출
입력: 유저스토리 + 고객 페르소나 (design/userstory.md, think/핵심솔루션.md)
...
**Step 6.1: 프로토타입 가이드**
```
16-prototype-development skill 호출
입력: 모든 이전 산출물 (design/uiux/uiux.md, design/uiux/style-guide.md, design/userstory.md, think/핵심솔루션.md)
...
2. **참여 유지**: 중간 산출물을 검토하고 피드백 제공
3. **질문하기**: 어느 단계에서든 설명을 요청
4. **반복하기**: 어떤 단계의 산출물이든 개선 요청
5. **작업 저장**: 모든 산출물이 증분적으로 저장됨
...
- 전체 프로세스는 약 95-115분 소요됩니다
- 각 단계의 산출물을 검토하고 필요시 수정을 요청하세요
- 중간에 중단된 경우 언제든 재개할 수 있습니다
- 모든 산출물은 자동으로 저장되므로 작업 내용이 보존됩니다

## 스킬 참조

이 오케스트레이션 스킬은 다음 16개의 전문화된 스킬을 사용합니다:

1. `01-mvp-definition`: MVP 주제 정의
2. `02-customer-analysis`: 타겟 고객 분석
3. `03-market-research`: 시장 조사 및 경쟁 분석
4. `04-customer-journey-stages`: 고객경험 단계 정의
5. `05-customer-experience`: 고객 경험 조사
6. `06-journey-mapping`: 고객 여정 맵 작성
7. `07-problem-hypothesis`: 문제 가설 정의
8. `08-direction-setting`: 킹핀 문제 및 방향성 정의
9. `09-ideation`: 아이디어 발상
10. `10-solution-selection`: 솔루션 선정
11. `11-business-modeling`: 비즈니스 모델 캔버스
12. `12-presentation`: 발표자료 스크립트
13. `13-event-storming`: 이벤트 스토밍 (PlantUML)
14. `14-user-stories`: 유저스토리 작성
15. `15-uiux-design`: UI/UX 설계
16. `16-prototype-development`: 프로토타입 개발
